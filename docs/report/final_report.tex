...
% ----- Forest node styles -----
\forestset{
  ornd/.style  = {ellipse, draw, thick, align=center, inner sep=3pt},
  andnd/.style = {rectangle, draw, thick, double, align=center,
...
\begin{forest}
  for tree={
    edge={-Latex},
    l sep=14pt, s sep=6pt,
    anchor=north,
  }
  [{\textbf{feasible(I,\,S)}\\[2pt]\scriptsize(AND: all checks pass)},
   andnd
    [{\(\lnot\)H1\\[-1pt]\scriptsize no missing\\[-2pt]\scriptsize lectures},
     ornd
      [{$\forall c:\;$\\[-1pt]\scriptsize lectures(c)\\[-2pt]\scriptsize covered}, lfnd]
    ]
    [{\(\lnot\)H2\\[-1pt]\scriptsize no room\\[-2pt]\scriptsize conflict},
     ornd
      [{$\forall (d,p,r):$\\[-1pt]\scriptsize $\leq1$ lecture\\[-2pt]\scriptsize per slot/room}, lfnd]
    ]
    [{\(\lnot\)H3\\[-1pt]\scriptsize no course\\[-2pt]\scriptsize self-conflict},
     ornd
      [{$\forall c,i\!\neq\!j:$\\[-1pt]\scriptsize diff slot\\[-2pt]\scriptsize per course}, lfnd]
    ]
    [{\(\lnot\)H4\\[-1pt]\scriptsize no teacher\\[-2pt]\scriptsize conflict},
     ornd
      [{$\forall t,(d,p):$\\[-1pt]\scriptsize $\leq1$ course\\[-2pt]\scriptsize per teacher}, lfnd]
    ]
    [{\(\lnot\)H5\\[-1pt]\scriptsize no curriculum\\[-2pt]\scriptsize conflict},
     ornd
      [{$\forall q,(d,p):$\\[-1pt]\scriptsize $\leq1$ course\\[-2pt]\scriptsize per curr.}, lfnd]
    ]
    [{\(\lnot\)H6\\[-1pt]\scriptsize no unavail-\\[-2pt]\scriptsize ability},
     ornd
...
\begin{tikzpicture}[baseline=(a.base)]
  \node[ellipse, draw, thick, inner sep=2pt, font=\small] (a) {OR node};
  \node[rectangle, draw, thick, double, rounded corners=2pt,
        inner sep=2pt, font=\small, right=10pt of a] (b) {AND node};
  \node[rectangle, draw, fill=gray!15, inner sep=2pt, font=\small\ttfamily,
        right=10pt of b] (c) {leaf};
...
\begin{forest}
  for tree={
    edge={-Latex},
    l sep=14pt, s sep=8pt,
    anchor=north,
  }
  [{\textbf{Produce Feasible}\\[-1pt]\textbf{Schedule}\\[2pt]
    \scriptsize(OR: either solver suffices)},
   ornd
    [{\textbf{Constructive}\\[-1pt]\textbf{Solver}\\[2pt]
      \scriptsize(AND: all phases)},
     andnd
      [{\scriptsize Parse\\[-1pt]\scriptsize Instance}, lfnd]
      [{\scriptsize Order Courses\\[-1pt]\scriptsize (MCF heuristic)}, lfnd]
      [{\textbf{Place All}\\[-1pt]\textbf{Lectures}\\[2pt]\scriptsize(AND)},
       andnd
        [{\textbf{Place}\\[-1pt]\textbf{Lecture\,\textit{i}}\\[2pt]
          \scriptsize(OR: any valid\\[-2pt]\scriptsize slot+room)},
         ornd
          [{\textbf{Valid}\\[-1pt]\textbf{Placement}\\[2pt]\scriptsize(AND: all checks)},
           andnd
    ...
    [{\textbf{CLP(FD)}\\[-1pt]\textbf{Solver}\\[2pt]
     ...
     andnd
      [{\scriptsize Parse\\[-1pt]\scriptsize Instance}, lfnd]
      [{\textbf{Post FD}\\[-1pt]\textbf{Constraints}\\[2pt]\scriptsize(AND)},
       andnd
